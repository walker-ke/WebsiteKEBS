...
class EGOSourceEasydiscuss
{
	...
	{
		$info = array();
		$info['name']	= 'easydiscuss';
		$info['title']	= 'EasyDiscuss';
		$info['compat']	= '3.2';
		$info['compat_start']	= '3.2';
	...
	public function getExist()
	{
		...
		$pref	= $db->getPrefix();
		if(in_array( $pref.'discuss_posts', $tables))
			return true;
		else
			return false;
	}
	...
	{			
		...
		if(isset($this->_cols['egstart'])) 
			$query->where('( '. $this->_cols['egstart'] .' = '.$db->Quote($nullDate).' OR '. $this->_cols['egstart'] .' <= NOW() )') ;
		...
		)
		{
			...
			$query->where( "(". $this->_cols['egtitle'] ." LIKE '{$str}' OR ". $this->_cols['egintro'] ." LIKE '{$str}' )");
		}
		...
		)
		{
			...
			$query->where( "(". $this->_cols['egtitle'] ." NOT LIKE '{$str}' AND ". $this->_cols['egintro'] ." NOT LIKE '{$str}' ) ");
		}
		...
		// Join over userinfo
		$query->join('LEFT', '#__discuss_users AS ii ON ii.id = ' . $this->_cols['egauthor_id']);
		
		// Join over the categories table for category name
		if(isset($this->_cols['egcat_id']))
		{
			$query->join('LEFT', '#__discuss_category AS cat ON cat.id = ' . $this->_cols['egcat_id']);
		...
		// die( nl2br(str_replace('#_', 'cuzaj', $query)) );
		return $query;
	}
	...
	{			
		...
		$query->select('a.id as id, a.title as title, parent_id AS parent, level');
	...
	public function getLink($id, $catid = null, $config= array()) 
	{
		...
		$url = 'index.php?option=com_easydiscuss&view=post&id=' . $id . '&Itemid=' . $Itemid;
		$url = JRoute::_($url);
	...
	public function getCatlink($catid = null, $catalias = null, $filter= array()) 
	{
		...
		$Itemid = DiscussRouter::getItemId();
		$url = 'index.php?option=com_easydiscuss&view=categories&layout=listings&category_id=' . $catid . '&Itemid=' . $Itemid;
		$url = JRoute::_($url);
		return $url;
	}
	...
	public function getImages($image = null, $text = null , $multi = false)
	{
		...
		$output['url'] = 'media/com_easydiscuss/images/' . $image; 
		if(isset($output['url']))
		{
