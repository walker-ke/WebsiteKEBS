...
class EGOSourceK2
{
	...
	public function getExist()
	{
		...
		$pref	= $db->getPrefix();
		if(in_array( $pref.'k2_items', $tables))
			return true;
		else
			return false;
	}
	...
	{			
		...
		)
		{
			...
			$query->where( $this->_cols['egtitle'] ." NOT LIKE '{$str}' ");
		}
		...
		{
			$query->join('LEFT', '#__k2_categories AS cat ON cat.id = ' . $this->_cols['egcat_id']);
	...
	{			
		...
		$query->select('a.id, a.name as title, a.parent');
	...
	public function getCatlink($catid = null, $catalias = null, $filter= array()) 
	{
		...
		$url = JRoute::_(K2HelperRoute::getCategoryRoute($catid));
	...
	public function getImages($image = null, $text = null , $multi = false)
	{
		...
		{
			$tmpimg = 'media/k2/items/src/'.md5('Image'.$image).'.jpg';
			if(is_file(JPATH_SITE . '/' . $tmpimg))
			{
					$output['url'] = $tmpimg;	
			}
		}
