...
<?php
	...
	$Html->addClass($this->Parser->parse($view['class']));
	...
	if(!empty($view['static']['popup']['enabled'])){
		...
		$popup = '<div class="ui fluid popup top left transition hidden G2-static-popup popup-'.$view['name'].'">'.$this->Parser->parse($view['static']['popup']['content']).'</div>';
	}
	...
	if(!empty($view['counter'])){
		$Html->attr('data-counter', $this->Parser->parse($view['counter']));
	}
