...
<?php
	...
	if(!empty($function['paging'])){
		...
		$Composer->page(implode('-', $paginate_alias), $Model, $this->Parser->parse($function['limit']));
		$this->Paginator->params['alias'] = implode('-', $paginate_alias);
	}else{
		$Model->limit($this->Parser->parse($function['limit']));
		
		if(!empty($function['offset'])){
			$Model->offset($this->Parser->parse($function['offset']));
		}
	}
