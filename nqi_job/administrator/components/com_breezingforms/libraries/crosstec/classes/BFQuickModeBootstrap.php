...
class BFQuickModeBootstrap{
	...
	function __construct( HTML_facileFormsProcessor $p ){
		...
		$this->bsClasses[4] = array(
			...
			'control-group' => '',
			'control-label' => $this->bsClassPrefix.'col-form-label',
			...
			'input-group-btn' => $this->bsClassPrefix.'input-group-prepend',
		...
		if(isset($dataObject['attributes']) && isset($dataObject['properties']) ){
			...
			if($mdata['type'] == 'page'){
				...
				if(trim($mdata['description'])!=''){
					echo '<div class="bfSectionDescription'.( $normal ? ' mb-4' : '' ) . (isset($this->rootMdata['themebootstrapUseWell']) && $this->rootMdata['themebootstrapUseWell'] ? ' ' . $this->bsClass('well').' '.$this->bsClass('well-small') : '').'">'."\n";
