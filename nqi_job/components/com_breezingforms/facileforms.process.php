			...
			if ($row->type == "Captcha") {
				...
				function bfCheckCaptcha(){
					if(checkFileExtensions()){
                                               ...
                                               ao.sndReq("get","'.Juri::root(true).'/index.php?raw=true&option=com_breezingforms&checkCaptcha=true&Itemid=0&tmpl=component&value="+document.getElementById("bfCaptchaEntry").value,"");
					}
				}';
				break;
					...
					if($cbResult['data']['default_lang_code_ignore']){
						...
						else
						{
							...
							foreach($codes As $code){
								if(strstr(strtolower($code), strtolower(trim(BFRequest::getCmd('lang','')))) !== false){
				...
				if (is_object($cbData) && $cbData->create_articles) {
					...
					if(JFactory::getApplication()->isClient('site') && BFRequest::getInt('Itemid',0)){
						if (version_compare($version->getShortVersion(), '3.2', '>=')) {
							...
							$item = $menu->getActive();
							if (is_object($item)) {
								BFRequest::setVar('cb_category_id', $item->params->get('cb_category_id', null));
								BFRequest::setVar('cb_controller', $item->params->get('cb_controller', null));
							}
							...
							BFRequest::setVar('cb_category_id', $params->get('cb_category_id', null));
							BFRequest::setVar('cb_controller', $params->get('cb_controller', null));
						}
					}
		...
		if (BFRequest::getVar('cb_controller',null) != 'edit' && $cbRecordId && is_array($cbResult) && isset($cbResult['data']) && isset($cbResult['data']['id']) && $cbResult['data']['id']) {
			...
			if( $cbResult['data']['force_login'] ){
				...
				if( !JFactory::getUser()->get('id', 0) ){
					...
					{
						JFactory::getApplication()->redirect(JRoute::_('index.php?option=com_user&view=login&Itemid='.BFRequest::getInt('Itemid', 0), false));
					}
					...
					{
						JFactory::getApplication()->redirect(JRoute::_('index.php?option=com_user&view=user&Itemid='.BFRequest::getInt('Itemid', 0), false));
					}
				}
			}
			...
			JFactory::getApplication()->redirect(JRoute::_('index.php?option=com_contentbuilder&controller=details&Itemid='.BFRequest::getInt('Itemid',0).'&backtolist=' . BFRequest::getInt('backtolist', 0) . '&id=' . $cbResult['data']['id'] . '&record_id=' . $cbRecordId . '&limitstart=' . BFRequest::getInt('limitstart', 0) . '&filter_order=' . BFRequest::getCmd('filter_order'), false), BFText::_('COM_CONTENTBUILDER_SAVED'));
		}
